...
if [ "$ENV" != "local" ] && [ ${#CANISTERS_TO_CREATE[@]} -gt 0 ] && [ -n "${DFX_WALLET_ID:-}" ] && [ -n "${DFX_IDENTITY_PEM:-}" ]; then
  ...
  echo "▶ Depositing ${_FUND} cycles (${#CANISTERS_TO_CREATE[@]} × 2.5T) for $DEPLOY_PRINCIPAL..."
  if dfx canister call um5iw-rqaaa-aaaaq-qaaba-cai deposit \
    ...
    echo "  ⚠️  Cycles ledger deposit failed — ensure wallet has ≥ 10T cycles for a fresh deploy."
  fi
fi
