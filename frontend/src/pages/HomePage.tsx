...
export default function HomePage() {
  ...
  return (
    <div style={{ background: S.white, minHeight: "100vh", overflowX: "hidden" }}>
      ...
      {/* ── HomeGentic promo ─────────────────────────────────────────────────── */}
      <section style={{ background: "#F0FDF4", borderTop: "1px solid #BBF7D0", borderBottom: "1px solid #BBF7D0", padding: isMobile ? "48px 20px" : "64px 48px" }}>
        <div style={{
          maxWidth:      1100,
          margin:        "0 auto",
          display:       "grid",
          ...
          <div>
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
              <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke={S.green} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                <path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/>
                <polyline points="9 22 9 12 15 12 15 22"/>
              </svg>
              <span style={{ fontFamily: S.sans, fontSize: "0.95rem", fontWeight: 800, color: S.green, letterSpacing: "-0.01em" }}>HomeGentic</span>
              <span style={{
                fontFamily:    S.sans,
                fontSize:      "0.65rem",
                fontWeight:    600,
            ...
            <h2 style={{ fontFamily: S.sans, fontSize: "clamp(1.5rem, 2.5vw, 2rem)", fontWeight: 800, color: S.dark, lineHeight: 1.2, marginBottom: 16, letterSpacing: "-0.02em" }}>
              Sell for more with a verified home record.
            </h2>
            <p style={{ fontFamily: S.sans, fontSize: "0.95rem", color: S.muted, lineHeight: 1.75, marginBottom: 28, maxWidth: 460 }}>
              Before you list on BidToList, give agents — and buyers — the full picture. HomeGentic is the Carfax for your home: every repair, renovation, and inspection documented and verified on the blockchain.
            </p>

            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
              {[
                "Verified maintenance history increases buyer confidence",
                "Documented records reduce back-and-forth during due diligence",
                "Homes with a HomeGentic record close faster and for more",
              ].map(point => (
                <div key={point} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={S.green} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 2 }}>
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                  <span style={{ fontFamily: S.sans, fontSize: "0.875rem", color: S.dark, lineHeight: 1.6 }}>{point}</span>
                </div>
              ))}
            </div>
          ...
          {!isMobile && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[
                { icon: "📋", title: "Verified Records",  desc: "Every repair and renovation, permanently logged." },
                { icon: "🎤", title: "AI Home Agent",     desc: "Voice queries across your full maintenance history." },
                { icon: "📊", title: "Home Score",        desc: "Live health score buyers and agents trust." },
                { icon: "👷", title: "Vetted Network",    desc: "Background-checked contractors, auto-logged work." },
              ].map(({ icon, title, desc }) => (
                <div key={title} style={{
                  background:   S.white,
                  ...
                  <p style={{ fontFamily: S.sans, fontSize: "0.875rem", fontWeight: 700, color: S.dark, marginBottom: 6 }}>{title}</p>
                  <p style={{ fontFamily: S.sans, fontSize: "0.78rem", color: S.muted, lineHeight: 1.6 }}>{desc}</p>
                </div>
              ))}
            </div>
          )}
